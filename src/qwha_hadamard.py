"""VENDORED from QWHA (ICLR'26) -- github.com/vantaa89/qwha,
peft/src/peft/tuners/qwha/hadamard.py.  Cloned to ./temp/qwha.
(Itself adapted by those authors from QuaRot, https://github.com/spcl/QuaRot.)

NOT REIMPLEMENTED.  Byte-identical to the authors' file except for this header and
the guarded import below; `src/verify_qwha_adapter.py` gate G0 asserts that.

⚠️ ONE DEVIATION, declared: the authors' CUDA fast path needs the compiled
`fast_hadamard_transform` extension (Dao-AILab), which is NOT installed here.  The
import is therefore made OPTIONAL and the pure-torch `matmul_hadU` path is used on
every device.  This changes SPEED, never MATH -- and this repo makes flop/parameter
claims, never throughput claims, so the substitution is inside our declared scope.
Gate G2 checks wht/iwht round-trip to fp64 precision on the torch path.
"""
# Adapted from https://github.com/spcl/QuaRot/blob/main/quarot/functional/hadamard.py
import torch, math
try:                                    # optional: CUDA fast path, see header
    import fast_hadamard_transform
    _HAS_FHT = True
except ImportError:                     # pure-torch fallback, math-identical
