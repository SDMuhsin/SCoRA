#!/bin/bash
# [R.306] failure watcher.  Emits ONLY lines worth acting on:
...
while true; do
  ...
  grep -E '^\[r306\] stage .* EXIT|ALL STAGES DONE|SELFTEST FAILED|stage B converged' "$D/run.log" 2>/dev/null \
    | comm -13 "$D/.seen_marks" - 2>/dev/null
  grep -E '^\[r306\] stage .* EXIT|ALL STAGES DONE|SELFTEST FAILED|stage B converged' "$D/run.log" 2>/dev/null > "$D/.seen_marks"
  nd=$(ls "$D/done" 2>/dev/null | wc -l)
  now=$(date +%s)
  [ "$nd" -ne "$last_done" ] && { last_done=$nd; last_change=$now; }
  if ! pgrep -f r306_run.sh > /dev/null; then
