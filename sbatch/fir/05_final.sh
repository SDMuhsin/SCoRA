#!/bin/bash
# ============================================================================
# 05_final.sh — THE FINAL MULTI-TASK RUNS on gemma-2b, at the FROZEN proxy
# hyperparameters (llmdocs/GEMMA_HP_PROXY.md).  ONE ARRAY PER TASK.
# ============================================================================
#   FIR_FINAL_TASK=rte bash sbatch/fir/05_final.sh --canary 9   # ⭐ ALWAYS FIRST
#   FIR_FINAL_TASK=rte bash sbatch/fir/05_final.sh --time HH:MM:SS --concurrent 16
#   FIR_FINAL_TASK=rte bash sbatch/fir/05_final.sh --status
#   FIR_FINAL_TASK=all bash sbatch/fir/05_final.sh --status      # every task
#   bash sbatch/fir/05_final.sh --dry-run                        # plan, submit nothing
#
...
#   calls `fir_hp_run_cell.py --planner final`, so the RECEIPT CHECK -- the one
...
GRID_NAME="${FIR_FINAL_TASK:-all}"
export FIR_FINAL_TASK="$GRID_NAME"
...
if [ "$GRID_NAME" = "all" ] && [ -z "$LOCAL_ONE" ] && ! $STATUS; then
    echo "⛔ FIR_FINAL_TASK=all is a READING VIEW, not a run target."
    echo "   --time is per-array and the per-task cost spans ~6x, so the six tasks"
    echo "   are six submissions. Pick one:"
    echo "     FIR_FINAL_TASK=rte bash sbatch/fir/05_final.sh --canary 9"
...
PLAN_FILE="$SWEEP_ROOT/plans/final-${GRID_NAME}-$(date -u +%Y%m%dT%H%M%SZ)-$$.txt"
...
if [ -n "$LOCAL_ONE" ]; then
    ...
    t0=$SECONDS
    "$PY_BIN" scripts/fir_hp_run_cell.py --planner final --cell "$cid" --run-root "$SWEEP_ROOT"
    rc=$?
    el=$((SECONDS - t0))
    ...
    if [ $rc -eq 5 ]; then
        ...
        { echo "exit=5 PLAN/GRID MISMATCH -- nothing ran. node=$(hostname)"
          echo "cell id '$cid' is not in task view '${FIR_FINAL_TASK:-?}'."
...
# STATUS
# ===========================================================================
env/bin/python scripts/fir_final_plan.py --list > "$PLAN_FILE" || exit 1
...
if $STATUS; then
    ...
    COLLECT="${FIR_COLLECT_DIR:-./logs/final}"; mkdir -p "$COLLECT"
    n_c=0
    ...
    echo
    echo "read: env/bin/python scripts/fir_final_read.py --run-root $SWEEP_ROOT --emit results/"
    exit 0
fi
...
echo
env/bin/python scripts/fir_final_plan.py --show
echo
...
if [ "$NDONE_ALL" -eq 0 ]; then
    ...
    for d in "$(dirname "$FIR_SCRATCH_ROOT")"/*/runs/final; do
        [ -d "$d/done" ] || continue
...
PY
    ...
    if [ -z "$ARRAY_SPEC" ]; then
        ...
        echo "  read: env/bin/python scripts/fir_final_read.py --run-root $SWEEP_ROOT"
        exit 0
    fi
...
cat > "$BODY_FILE" <<SB
#!/bin/bash
#SBATCH --job-name=lrs_final_${GRID_NAME}
...
bash sbatch/fir/05_final.sh --run-one "\$cid"
SB
...
echo "status:  FIR_FINAL_TASK=$GRID_NAME bash sbatch/fir/05_final.sh --status"
echo
if [ "$P_CANARY" -gt 0 ]; then
